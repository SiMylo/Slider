...
class Piece:
  ...
  def __init__(self,shape,colors,isBottom=False,invisible=False):
    ...
    if shape == "triangle":
      uVerts.append((0,0,0))
      uVerts.append((TRIANGLE_LENGTH*cos(radians(BASIC_ANGLE)),0,TRIANGLE_LENGTH*sin(radians(BASIC_ANGLE))))
      uVerts.append((TRIANGLE_LENGTH*cos(radians(-BASIC_ANGLE)),0,TRIANGLE_LENGTH*sin(radians(-BASIC_ANGLE))))
      ...
      self.segments.append(Segment([uVerts[0],uVerts[1],dVerts[1],dVerts[0]],Piece.BLACK))
      self.segments.append(Segment([uVerts[0],uVerts[2],dVerts[2],dVerts[0]],Piece.BLACK))
      self.angularWidth = 2*BASIC_ANGLE
    elif shape == "corner":
      uVerts.append((0,0,0))
      uVerts.append((TRIANGLE_LENGTH*cos(radians(+BASIC_ANGLE*2)),0,(TRIANGLE_LENGTH*sin(radians(+BASIC_ANGLE*2)))))
      ...
      self.segments.append(Segment(dVerts,Piece.BLACK))
      self.segments.append(Segment([uVerts[0],uVerts[1],dVerts[1],dVerts[0]],Piece.BLACK))
      self.segments.append(Segment([uVerts[0],uVerts[3],dVerts[3],dVerts[0]],Piece.BLACK))
      self.angularWidth = 4*BASIC_ANGLE
    elif shape == "middle":
      mVerts.append((TRIANGLE_LENGTH*cos(radians(BASIC_ANGLE)),(TRIANGLE_LENGTH*sin(radians(BASIC_ANGLE))),CUBE_HEIGHT/2))
      mVerts.append((TRIANGLE_LENGTH*cos(radians(180-BASIC_ANGLE)),(TRIANGLE_LENGTH*sin(radians(180-BASIC_ANGLE))),CUBE_HEIGHT/2))
      mVerts.append((TRIANGLE_LENGTH*cos(radians(180+BASIC_ANGLE)),(TRIANGLE_LENGTH*sin(radians(180+BASIC_ANGLE))),CUBE_HEIGHT/2))
      mVerts.append((TRIANGLE_LENGTH*cos(radians(-BASIC_ANGLE)),(TRIANGLE_LENGTH*sin(radians(-BASIC_ANGLE))),CUBE_HEIGHT/2))
      self.segments.append(Segment(mVerts,self.colors[1]))
      lVerts.append(mVerts[2])
      lVerts.append(mVerts[1])
      lVerts.append((mVerts[1][0],mVerts[1][1],mVerts[1][2]-(CUBE_HEIGHT-CORNER_WIDTH)))
      lVerts.append((mVerts[2][0],mVerts[2][1],mVerts[2][2]-(CUBE_HEIGHT-CORNER_WIDTH)))
      ...
      self.segments.append(Segment(rVerts,self.colors[2]))
      self.segments.append(Segment([lVerts[2],lVerts[3],rVerts[2],rVerts[3]],Piece.BLACK))
      self.segments.append(Segment([mVerts[0],mVerts[1],lVerts[2],rVerts[3]],Piece.BLACK))
      self.segments.append(Segment([mVerts[2],mVerts[3],rVerts[2],lVerts[3]],Piece.BLACK))
...
class SquareOne:
  ...
  def setState(self, data):
    ...
    for piece in data['bottom']:
      self.bottomPieces.append(Piece(piece["shape"],piece["colors"],isBottom=True))
...
class MoveTracker(pyglet.window.Window):
  ...
  def on_draw(self):
    self.clear()
    self.moveText.text = "{}:\n\n{}".format(self.currentMove["name"],self.currentMove["moves"])
    self.moveText.draw()
...
if __name__ == '__main__':
  ...
  secondEndWindow = CubeWindow(400, 400, 'Square One Ending Back View',endCube,opposite=True)
  ...
  secondWindow = CubeWindow(400, 400, 'Square One Starting Back View',stateCube,opposite=True)
